      ...
      <main>
        <section className="border-b border-[#e2e8f0]">
          <div className="mx-auto max-w-7xl px-5 py-14 md:px-6 md:py-20 lg:px-8">
            <p className="text-xs font-medium uppercase tracking-[0.22em] text-[#64748b]">§ Thư viện mở</p>
            <div className="mt-4 grid gap-6 md:grid-cols-[minmax(0,1fr)_300px] md:items-end">
              <h1 className="max-w-4xl text-6xl font-black leading-[0.9] tracking-[-0.075em] md:text-7xl">
                Khám phá
                <span className="block font-serif font-normal italic text-[#64748b]">tri thức mở.</span>
              </h1>
          ...
          <div className="mx-auto grid max-w-7xl gap-3 px-5 py-5 md:grid-cols-[minmax(0,1fr)_220px] md:px-6 lg:px-8">
            <div className="relative">
              ...
              <input
                ...
                maxLength={128}
                className="w-full rounded-full border border-[#cbd5e1] bg-[#f8fafc] py-3 pl-11 pr-4 text-sm outline-none focus:border-[#64748b]"
              />
            </div>
            <select
              value={sortBy}
              onChange={(event) => setSortBy(event.target.value as SortOption)}
              className="rounded-full border border-[#cbd5e1] bg-[#f8fafc] px-4 py-3 text-sm outline-none focus:border-[#64748b]"
            >
        ...
        <section className="mx-auto max-w-5xl px-5 py-10 md:px-6 md:py-14">
          <div className="mb-5 flex items-center justify-between gap-4">
            ...
            {!currentUser && <p className="text-xs text-[#64748b]">Đăng nhập để mở chi tiết và tải PDF.</p>}
          </div>
          ...
          {!isLoading && papers.length === 0 && !error && (
            <div className="border border-[#e2e8f0] bg-white p-10 text-center">
              <Search size={34} className="mx-auto text-[#94a3b8]" />
              <p className="mt-4 text-sm text-[#64748b]">Không tìm thấy tài liệu phù hợp.</p>
            </div>
          )}
